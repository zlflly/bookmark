...
async function runFetchRequest(request, env, ctx, exists, getByETag) {
  ...
  try {
    env.JAEGER ??= mockJaegerBinding(), sentry = setupSentry(
      request,
      ctx,
      env.SENTRY_DSN,
...
var AssetWorkerOuter = class extends WorkerEntrypoint {
  ...
  async fetch(request) {
    ...
    try {
      ...
      ), await this.getInnerEntrypoint().fetch(request);
    } catch (err) {
      let response = handleError(sentry, analytics, err);
      return submitMetrics(analytics, performance, startTimeMs), response;
    }
  }
  async unstable_canFetch(request) {
    return this.env.JAEGER ??= mockJaegerBinding(), this.getInnerEntrypoint().unstable_canFetch(request);
  }
  async unstable_getByETag(eTag, request) {
    return this.env.JAEGER ??= mockJaegerBinding(), this.getInnerEntrypoint().unstable_getByETag(eTag, request);
  }
  async unstable_getByPathname(pathname, request) {
    return this.env.JAEGER ??= mockJaegerBinding(), this.getInnerEntrypoint().unstable_getByPathname(pathname, request);
  }
  async unstable_exists(pathname, request) {
    return this.env.JAEGER ??= mockJaegerBinding(), this.getInnerEntrypoint().unstable_exists(pathname, request);
  }
}, AssetWorkerInner = class extends WorkerEntrypoint {
  async fetch(request) {
    this.env.JAEGER ??= mockJaegerBinding();
    let traceContext = this.ctx.props?.traceContext ?? null, response = await this.env.JAEGER.runWithSpanContext(
      traceContext,
      () => runFetchRequest(
        request,
        this.env,
